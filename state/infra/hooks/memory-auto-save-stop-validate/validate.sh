#!/usr/bin/env bash
# memory-auto-save-stop-validate — Stop (Layer 7 of 10)
set -uo pipefail
...
V="$ROOT/state/memory_auto_save/violations.jsonl"
STALE_SEC=600
NOW=$(date +%s)
[ -f "$HB" ] || exit 0
...
if [ "$AGE" -gt "$STALE_SEC" ]; then
    ...
    echo "[memory-auto-save-stop-validate] WARN: heartbeat stale (${AGE}s)" >&2
fi
exit 0
