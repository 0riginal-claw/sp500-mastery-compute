"""
backtest_xgb_v6.py — ULTRA-maximal XGBoost pipeline.

All available feature layers (v5 stack + 4 new modules):
  - v5 base 587 features (technical, intraday, alt-data, trading-insight parts 1-4, v3 hand-coded)
  - cross-sectional 17 features (NEW, cache-loaded fast)
  - macro 40 features (NEW, yfinance no-key)
  - strategy signal 10 features + 5-filter stack 15 features (NEW)
  - google trends 7 features (NEW)
  - insider Form 4 8 features (NEW)

Total: ~684 features. Same scout-prune-refit pattern as v5 (top-K=50).

NOTE: multi_timeframe_features module not yet available; will add when ready.
"""
...
try:
    from alpaca_features import add_alpaca_features as _add_alpaca
except Exception as e:
    print(f"[warn] alpaca_features: {e}", file=sys.stderr); _add_alpaca = None
...
def build_v6_features(ticker: str, universe_agg: dict = None) -> pd.DataFrame:
    d = bml.load_daily(ticker)
    print(f"  [v6] base daily: {len(d):,}")
    ...
    if _add_alpaca is not None:
        try: f = _add_alpaca(f, ticker); print(f"    +alpaca: {f.shape[1]}")
        except Exception as e: print(f"    [w] alpaca: {e}")
...
def main():
    ...
    f = build_v6_features(args.ticker, universe_agg)
    fc = numeric_cols(f)
    ...
    meta = to_py({
        'ticker': args.ticker, 'pipeline_version': 'xgb_v6_ultramaximal',
        'strategy_variant': 'ML_XGB_v6_ultramaximal',
        ...
        'strategy': {'name':'ML_XGB_v6_ultramaximal', 'side':'long',
