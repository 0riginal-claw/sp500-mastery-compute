...
def main():
    ...
    for fold in folds:
        ...
        # Wave 7: XGBoost default + native predict_proba (already sigmoid-calibrated)
        base = xgb.XGBClassifier(max_depth=4, learning_rate=0.05, n_estimators=100,
                                 tree_method='hist', eval_metric='logloss',
                                 n_jobs=1, random_state=42, verbosity=0)
        base.fit(X_tr, y_tr)
        oos_probs = base.predict_proba(X_oos)[:, 1]
    ...
    meta = to_py({
        'ticker': args.ticker, 'pipeline_version': 'xgb_v1',
        'strategy_variant': 'ML_XGB_21d',
        ...
        'strategy': {'name': 'ML_XGB_21d', 'side': 'long', 'tp_atr': args.tp_atr, 'sl_atr': args.sl_atr,
                     'max_hold_days': args.max_hold, 'prob_threshold': args.prob_threshold,
                     'model': 'XGBClassifier', 'calibration': 'native XGBoost predict_proba',
